...
from app.sheets import EventSheets
import os
#ler caminho do argumento
parser = ArgumentParser()
parser.add_argument('--p', action='store', dest='path', help='Para pegar o caminho do arquivo "events.dart", usar pwd no código do app', default="/Users/andre/Documents/Projetos Flutter/konsi-app")
    ...
    for event in events:
        ...
        if event.startswith('///DESCRIPTION'):
            ...
            if 'opened' in descripted_event:
                event_type = 'View'
            formatted_events.append([descripted_event, 'Ambos', event_type, description])
        else:
            if 'click' in event:
                event_type = 'Click'
            if 'opened' in event:
            ...
            if all(event not in new_events[0]for new_events in formatted_events):
                formatted_events.append([event, 'Ambos', event_type, description])
